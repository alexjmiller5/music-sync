...
def test_r2_and_modal_contracts_coexist(provision, monkeypatch, capsys):
    monkeypatch.setattr(provision.sys, "argv", ["provision.py", "--list"])
    provision.main()
    fields = capsys.readouterr().out.splitlines()
    assert set(provision.MODAL_FIELDS) <= set(fields)
    assert "R2_API_TOKEN" in fields
    monkeypatch.setattr(provision, "mint_r2_token", lambda: "fixture-r2")
    monkeypatch.setattr(provision.sys, "argv", ["provision.py", "--field", "R2_API_TOKEN"])
    provision.main()
    assert capsys.readouterr().out == "fixture-r2\n"
